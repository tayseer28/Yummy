export class Area {
    #areaName = "";
    #areas = [];
    constructor() {
        this.showAreas(this.buildApi(this.#areaName));
        // Remove the extra closing brace
    }
    async showAreas(apiUrl, sectionId = "#areaSection") {
        try {
            let areas = await this.getAreas(apiUrl);
            this.#areas = areas;
            // console.log(areas);
            let content = ``;
            areas.forEach(area => {
                content += `
                <div class="rounded-lg group flex flex-col justify-center items-center">
                    
                     <i class="fa-solid fa-house-laptop text-7xl block text-white text-center"></i>
                    <h2 class = "text-white text-center font-bold text-2xl">${area.strArea}</h2>
                  
                </div>
                `});
            $(`${sectionId} .container .grid-content`).html(content);

        }
        catch (error) {
            console.log(error);
        }
    }
    async showMealsOfArea(areaName) {
        // let meals = this.#areas.find(area => area.strArea === areaName);
        // console.log(meals);
        // let content = ``;
        // meals.forEach(meal => {
        //     content += `
        //         <figure class="relative rounded-lg group">
        //           <img src="${meal.strMealThumb}" class="w-full" alt="${meal.strMeal}">
        //           <figcaption class=" ">
        //             <p>${meal.strMeal}</p>
        //           </figcaption>
        //         </figure>
        //         `});
        // $(`#mealsOfArea .container .grid-content`).html(content);    
        let meals = await this.getMealsByArea(areaName);
        meals = Array.from(meals || []); // Ensure meals is an array
        console.log(meals);
        let content = ``;
        meals.forEach(meal => {
            content += `
                <figure class="relative rounded-lg group">
                  <img src="${meal.strMealThumb}" class="w-full" alt="${meal.strMeal}">
                  <figcaption class=" flex justify-center items-center">
                    <p class = "text-black font-bold text-2xl">${meal.strMeal}</p>
                  </figcaption>
                </figure>
                `;
        });
        $(`#mealsOfArea .container .grid-content`).html(content);
    }
    async showMealOfAreaDetails(mealName) {
        //get meals array of that url: https://www.themealdb.com/api/json/v1/1/search.php?s={mealName}
        let url = `https://www.themealdb.com/api/json/v1/1/search.php?s=${mealName}`;
        try {
            let meal = await fetch(url);
            let data = await meal.json();
            console.log(data);
            let details = ``;
            //print the meal details
            details += `
            <!-- meal image -->
          <div class="meal-image md:w-2/5">
                    <img src="${data.meals[0].strMealThumb}" class="block w-full mb-1 rounded-lg" alt="${data.meals[0].strMeal}">
                    <h2 class="text-2xl font-bold mb-4 text-white">${data.meals[0].strMeal}</h2>
                </div>

                <!-- meal details -->
     <div class="meal-details md:w-3/5">
            <div class="w-full p-4 text-white">
              <h3 class="text-3xl font-semibold mb-3">Instructions:</h3>
              <p class="mb-4 mt-4 tracking-wide">${data.meals[0].strInstructions}</p>
              <p class="mb-2 text-3xl font-bold">Area: ${data.meals[0].strArea}</p>
              <p class="mb-4 font-bold text-3xl">Category: ${data.meals[0].strCategory}</p>
              <div class="recipe mb-4">
                <p class="font-semibold text-3xl mb-2">Recipes:</p>
                <div class="flex flex-wrap">
                `;
            for (let i = 1; i <= 20; i++) {
                let ingredient = data.meals[0][`strIngredient${i}`];
                let measure = data.meals[0][`strMeasure${i}`];
                if (ingredient && ingredient.trim() !== "") {//we use trim to remove any white space
                    details += `<span class="py-2 px-1 bg-[#CFF4FC] text-[#055160] m-2 rounded-xl">${measure}  ${ingredient}</span>`;
                } else {
                    break;
                }
            }
            details += `
            </div>
          </div>`
        if (data.meals[0].strTags) {
            let tags = data.meals[0].strTags.split(",");
            details += `<div class="tags mb-4">

                         <p class="font-semibold  text-2xl">Tags:</p>`
            tags.forEach(tag => {
                details += `<span class = "p-2 bg-red-200 rounded-lg mt-3 me-3 inline-block">${tag}</span>`;
            });
            details += `</div> `
            details += `            
          <div class="group-button flex space-x-4">
            <button class="p-3 bg-[#198754] hover:bg-[#13653f] transition-all duration-300 text-black rounded" onclick="window.open('${data.meals[0].strSource}', '_blank')" >Source</button>
            <button class="p-3 bg-red-500 text-white rounded" onclick ="window.open('${data.meals[0].strYoutube}')">YouTube</button>
          </div>
        </div>
    </div>
  </div>`
            $("#mealDetail #mainContent").html(details);
        }
        }
        catch (error) {
            console.log(error);
        }
    }

    buildApi(areaName = "") {
        this.#areaName = areaName;
        let url = this.#areaName !== ""
            ? `https://www.themealdb.com/api/json/v1/1/filter.php?a=${this.#areaName}` :
            `https://www.themealdb.com/api/json/v1/1/list.php?a=list`;
        console.log(url);
        return url;
    }
    async getMealsByArea(areaName) {

        let url = `https://www.themealdb.com/api/json/v1/1/filter.php?a=${areaName}`;
        try {
            let response = await fetch(url);
            let data = await response.json();
            return data.meals;
        }
        catch (error) {
            console.log(error);
        }
    }
    async getAreas(url) {
        try {
            let response = await fetch(url);
            let areas = await response.json();
            // console.log(areas.meals);
            return areas.meals;
        }
        catch (error) {
            console.log(error);
        }
    }
}
